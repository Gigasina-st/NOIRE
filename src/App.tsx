...
function AppInner(){const [menu,setMenu]=useState(false),[search,setSearch]=useState(false),[cart,setCart]=useState(false),[wishlist,setWishlist]=useState(false),[collection,setCollection]=useState(false),[account,setAccount]=useState(false);const productSlugRoute=useProductRoute();const products=useProducts();const routeProduct=productSlugRoute?products.find(p=>productSlug(p)===productSlugRoute):null;const isCheckout=window.location.pathname==='/checkout';useCanonicalMeta(routeProduct);const openProduct=(p:Product)=>goToProduct(p);const backFromProduct=()=>{window.history.pushState({},'', '/');window.dispatchEvent(new PopStateEvent('popstate'))};const backFromCheckout=()=>{window.history.pushState({},'', '/');window.dispatchEvent(new PopStateEvent('popstate'))};if(isCheckout)return <><Checkout close={backFromCheckout} onAccount={()=>setAccount(true)}/>{account&&<Account close={()=>setAccount(false)} onCheckout={goToCheckout}/>}</>;if(productSlugRoute&&!routeProduct){return products.length?<NotFound/>:<main/>}if(window.location.pathname!=='/'&&window.location.pathname!==''&&!productSlugRoute){return <NotFound/>}if(productSlugRoute&&routeProduct){return <><Header onMenu={()=>setMenu(true)} onSearch={()=>setSearch(true)} onCart={()=>setCart(true)} onWishlist={()=>setWishlist(true)} onAccount={()=>setAccount(true)} onCollections={()=>setCollection(true)}/><ProductPage product={routeProduct} back={backFromProduct}/><Footer/></>}return <><Header onMenu={()=>setMenu(true)} onSearch={()=>setSearch(true)} onCart={()=>setCart(true)} onWishlist={()=>setWishlist(true)} onAccount={()=>setAccount(true)} onCollections={()=>setCollection(true)}/>{menu&&<MobileNav close={()=>setMenu(false)} onSearch={()=>setSearch(true)} onCart={()=>setCart(true)} onWishlist={()=>setWishlist(true)} onAccount={()=>setAccount(true)} onCollections={()=>setCollection(true)}/>} {search&&<SearchOverlay close={()=>setSearch(false)} onOpen={openProduct}/>} {cart&&<CartDrawer close={()=>setCart(false)} onCheckout={goToCheckout}/>} {wishlist&&<WishlistDrawer close={()=>setWishlist(false)} onOpen={openProduct}/>} {account&&<Account close={()=>setAccount(false)} onCheckout={goToCheckout}/>} {collection&&<CollectionPage close={()=>setCollection(false)} openProduct={openProduct}/>}<main><Hero/><Campaign01 onDiscover={()=>setCollection(true)}/><Featured openProduct={openProduct} onCollections={()=>setCollection(true)}/><NewArrivals openProduct={openProduct}/><Story/><Editorial/><Journal/><Newsletter/></main><Footer/>}
