...
const STORE_CONTACT={phone:'+989120000000',email:'hello@noire.example',instagram:'https://instagram.com/noire_official',whatsapp:'https://wa.me/989120000000',address:'Tehran, Iran'};
function FaqPanel({close}:{close:()=>void}){useEscape(close);return <div className="service-layer"><button className="service-backdrop" onClick={close}/><aside className="service-panel"><div className="service-head"><div><p className="eyebrow">NOIRÉ / FAQ</p><h2>Questions,<br/><em>answered.</em></h2></div><button onClick={close}><X size={22}/></button></div><div className="service-list"><details open><summary>How long does delivery take?</summary><p>Orders are prepared with care and dispatched as soon as they are ready.</p></details><details><summary>Can I change or cancel an order?</summary><p>Contact NOIRÉ as soon as possible with your order number.</p></details><details><summary>How do I choose my size?</summary><p>Use the size selector on each product page. A detailed size guide can be added here later.</p></details><details><summary>How can I track my order?</summary><p>Sign in to your account to view your order history and latest status.</p></details><details><summary>Do you ship internationally?</summary><p>International shipping information can be added here later.</p></details></div></aside></div>}
function ContactPanel({close}:{close:()=>void}){useEscape(close);return <div className="service-layer"><button className="service-backdrop" onClick={close}/><aside className="service-panel contact-panel"><div className="service-head"><div><p className="eyebrow">NOIRÉ / CONTACT</p><h2>Talk to<br/><em>the house.</em></h2></div><button onClick={close}><X size={22}/></button></div><div className="contact-grid"><a href={`tel:${STORE_CONTACT.phone}`}><span>PHONE</span><strong>{STORE_CONTACT.phone}</strong><small>Tap to call</small></a><a href={`mailto:${STORE_CONTACT.email}`}><span>EMAIL</span><strong>{STORE_CONTACT.email}</strong><small>Tap to email</small></a><a href={STORE_CONTACT.whatsapp} target="_blank" rel="noreferrer"><span>WHATSAPP</span><strong>Message us</strong><small>Open WhatsApp</small></a><a href={STORE_CONTACT.instagram} target="_blank" rel="noreferrer"><span>INSTAGRAM</span><strong>@noire_official</strong><small>Open Instagram</small></a><a className="contact-address" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(STORE_CONTACT.address)}`} target="_blank" rel="noreferrer"><span>ADDRESS</span><strong>{STORE_CONTACT.address}</strong><small>Open in Maps</small></a><div><span>OPENING HOURS</span><strong>Mon — Sat · 10:00 — 19:00</strong><small>Local time</small></div></div><p className="contact-note">These contact details are temporary placeholders; replace them with the seller's real details later.</p></aside></div>}
function Footer({onHomeSection,onAccount,onFaq,onContact}:{onHomeSection?:(section:string)=>void;onAccount?:()=>void;onFaq?:()=>void;onContact?:()=>void}){const link=(section:string)=>(e:React.MouseEvent<HTMLAnchorElement>)=>{if(onHomeSection){e.preventDefault();onHomeSection(section)}};return <footer><div className="footer-top"><a className="wordmark" href={onHomeSection?"/#top":"#top"} onClick={onHomeSection?link('top'):undefined}>NOIRÉ</a><p>A modern house for<br/>quietly remarkable things.</p><div className="footer-links"><div><span>SHOP</span><a href={onHomeSection?"/#shop":"#shop"} onClick={onHomeSection?link('shop'):undefined}>New arrivals</a><a href={onHomeSection?"/#collections":"#collections"} onClick={onHomeSection?link('collections'):undefined}>Collections</a><a href={onHomeSection?"/#shop":"#shop"} onClick={onHomeSection?link('shop'):undefined}>Accessories</a></div><div><span>HOUSE</span><a href={onHomeSection?"/#about":"#about"} onClick={onHomeSection?link('about'):undefined}>Our story</a><a href={onHomeSection?"/#journal":"#journal"} onClick={onHomeSection?link('journal'):undefined}>Journal</a><a href={onHomeSection?"/#about":"#about"} onClick={onHomeSection?link('about'):undefined}>Stockists</a></div><div><span>CARE</span><button type="button" onClick={onAccount}>Shipping & returns</button><button type="button" onClick={onContact}>Contact</button><button type="button" onClick={onFaq}>FAQ</button></div></div></div><div className="footer-bottom"><span>© 2026 NOIRÉ PARIS</span><div><a href={onHomeSection?"/#top":"#top"} onClick={onHomeSection?link('top'):undefined}>Instagram</a><a href={onHomeSection?"/#top":"#top"} onClick={onHomeSection?link('top'):undefined}>Pinterest</a><a href={onHomeSection?"/#top":"#top"} onClick={onHomeSection?link('top'):undefined}>Legal</a></div><span>Made with intention.</span></div></footer>}
