...
function useProductRoute(){
  const [slug,setSlug]=useState(()=>window.location.pathname.match(/^\/product\/([^/]+)\/?$/)?.[1]||null);
  useEffect(()=>{
    const sync=()=>setSlug(window.location.pathname.match(/^\/product\/([^/]+)\/?$/)?.[1]||null);
    addEventListener('popstate',sync);
    return()=>removeEventListener('popstate',sync);
  },[]);
  return slug;
}
function Header({onMenu,onSearch,onCart,onWishlist,onCollections,onAccount}:{onMenu:()=>void;onSearch:()=>void;onCart:()=>void;onWishlist:()=>void;onCollections:()=>void;onAccount:()=>void}){const [scrolled,setScrolled]=useState(false);const {cartCount,wishlist}=useStore();useEffect(()=>{const f=()=>setScrolled(scrollY>30);addEventListener('scroll',f,{passive:true});f();return()=>removeEventListener('scroll',f)},[]);return <header className={'site-header '+(scrolled?'is-scrolled':'')}><a className="wordmark" href="#top">NOIRÉ</a><nav className="desktop-nav">{nav.map(x=>x==='Collections'?<button key={x} onClick={onCollections}>{x}</button>:<a key={x} href={'#'+x.toLowerCase()}>{x}</a>)}</nav><div className="header-actions"><button onClick={onSearch} aria-label="Search"><Search size={16}/></button><button onClick={onWishlist} aria-label="Wishlist"><Heart size={16}/>{wishlist.length>0&&<span>{wishlist.length}</span>}</button><button onClick={onAccount} aria-label="Account"><UserRound size={16}/></button><button onClick={onCart} className="cart-button" aria-label="Cart"><ShoppingBag size={16}/><span>{cartCount}</span></button></div><button className="mobile-menu" onClick={onMenu} aria-label="Open menu"><Menu size={21}/></button></header>}

function MobileNav({close,onSearch,onCart,onWishlist,onCollections,onAccount}:{close:()=>void;onSearch:()=>void;onCart:()=>void;onWishlist:()=>void;onCollections:()=>void;onAccount:()=>void}){useLock(true);useEscape(close);return <div className="mobile-overlay"><div className="mobile-top"><span className="wordmark">NOIRÉ</span><button onClick={close}><X size={23}/></button></div><nav>{nav.map((x,i)=>x==='Collections'?<button className="mobile-nav-link" key={x} onClick={()=>{close();onCollections()}}><span>0{i+1}</span>{x}<ArrowUpRight size={18}/></button>:<a key={x} href={'#'+x.toLowerCase()} onClick={close}><span>0{i+1}</span>{x}<ArrowUpRight size={18}/></a>)}</nav><div className="mobile-tools"><button onClick={()=>{close();onSearch()}}><Search size={15}/> Search</button><button onClick={()=>{close();onWishlist()}}><Heart size={15}/> Wishlist</button><button onClick={()=>{close();onCart()}}><ShoppingBag size={15}/> Bag</button><button onClick={()=>{close();onAccount()}}><UserRound size={15}/> Account</button></div><p>Paris · London · Everywhere</p></div>}
...
function ProductGallery({product}:{product:Product}){const [index,setIndex]=useState(0);const [zoom,setZoom]=useState(false);const image=product.gallery[index]||product.image;return <div className="gallery"><div className="gallery-main" onClick={()=>setZoom(true)}><img src={image} alt={product.alt} loading="eager" decoding="async"/><span><ZoomIn size={14}/> View full</span><button className="gallery-prev" onClick={e=>{e.stopPropagation();setIndex((index-1+product.gallery.length)%product.gallery.length)}}><ArrowLeft size={16}/></button><button className="gallery-next" onClick={e=>{e.stopPropagation();setIndex((index+1)%product.gallery.length)}}><ArrowRight size={16}/></button></div><div className="gallery-thumbs">{product.gallery.map((src,i)=><button className={index===i?'active':''} key={src} onClick={()=>setIndex(i)}><img src={src} alt="" loading="lazy" decoding="async"/></button>)}</div>{zoom&&<div className="zoom-view" onClick={()=>setZoom(false)}><img src={image} alt={product.alt}/><button onClick={()=>setZoom(false)}><X size={21}/></button></div>}</div>}
...
function AppInner(){const [menu,setMenu]=useState(false),[search,setSearch]=useState(false),[cart,setCart]=useState(false),[wishlist,setWishlist]=useState(false),[collection,setCollection]=useState(false),[account,setAccount]=useState(false);const productSlugRoute=useProductRoute();const products=useProducts();const routeProduct=productSlugRoute?products.find(p=>productSlug(p)===productSlugRoute):null;const isCheckout=window.location.pathname==='/checkout';useCanonicalMeta(routeProduct);const openProduct=(p:Product)=>goToProduct(p);const backFromProduct=()=>{window.history.pushState({},'', '/');window.dispatchEvent(new PopStateEvent('popstate'))};const backFromCheckout=()=>{window.history.pushState({},'', '/');window.dispatchEvent(new PopStateEvent('popstate'))};if(isCheckout)return <Checkout close={backFromCheckout} onAccount={()=>setAccount(true)}/>;if(productSlugRoute&&!routeProduct){return products.length?<NotFound/>:<main/>}if(window.location.pathname!=='/'&&window.location.pathname!==''&&!productSlugRoute){return <NotFound/>}if(productSlugRoute&&routeProduct){return <><Header onMenu={()=>setMenu(true)} onSearch={()=>setSearch(true)} onCart={()=>setCart(true)} onWishlist={()=>setWishlist(true)} onAccount={()=>setAccount(true)} onCollections={()=>setCollection(true)}/><ProductPage product={routeProduct} back={backFromProduct}/><Footer/></>}return <><Header onMenu={()=>setMenu(true)} onSearch={()=>setSearch(true)} onCart={()=>setCart(true)} onWishlist={()=>setWishlist(true)} onAccount={()=>setAccount(true)} onCollections={()=>setCollection(true)}/>{menu&&<MobileNav close={()=>setMenu(false)} onSearch={()=>setSearch(true)} onCart={()=>setCart(true)} onWishlist={()=>setWishlist(true)} onAccount={()=>setAccount(true)} onCollections={()=>setCollection(true)}/>} {search&&<SearchOverlay close={()=>setSearch(false)} onOpen={openProduct}/>} {cart&&<CartDrawer close={()=>setCart(false)} onCheckout={goToCheckout}/>} {wishlist&&<WishlistDrawer close={()=>setWishlist(false)} onOpen={openProduct}/>} {account&&<Account close={()=>setAccount(false)} onCheckout={goToCheckout}/>} {collection&&<CollectionPage close={()=>setCollection(false)} openProduct={openProduct}/>}<main><Hero/><Campaign01 onDiscover={()=>setCollection(true)}/><Featured openProduct={openProduct} onCollections={()=>setCollection(true)}/><NewArrivals openProduct={openProduct}/><Story/><Editorial/><Journal/><Newsletter/></main><Footer/>}
